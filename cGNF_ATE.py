...
import pandas as pd  
import os 
os.environ["CUDA_VISIBLE_DEVICES"] = '0'  # Setting the environment variable to choose the first GPU for CUDA to use.
...
np.random.seed(2813308004) # Sets the seed for the simulation.

A = np.random.binomial(n=1, p=0.5, size=obs)

Y = np.array([np.random.binomial(n=1, p=0.6) if a == 1 else np.random.binomial(n=1, p=0.4) for a in A])

df = pd.DataFrame(data={'A':A, 'Y':Y}) 
...
process(path=path, dataset_name=dataset_name, dag_name=dataset_name + '_DAG', test_size=0.2, cat_var=['A', 'Y'], seed=None)
...
sim(path=path, dataset_name=dataset_name, model_name='20k', n_mce_samples=50000, inv_datafile_name='sim_20k',
        treatment='A', cat_list=[0, 1], moderator=None, mediator=None, outcome='Y')
